...
| building-plugin   | Bool     | Determines whether ot not it is a plugin what is going to be build                                        |
| enable-plugins-list| String | String containing the plugins to be enable separated by spaces, this include the plugin being built and its requirements. For example if we want to build the ecommerce plugin we will have to pass "discovery ecommerce mfe" because ecommerce depends on the discovery and mfe plugins.|
| plugin-package | String | String containing the name of the pypi package of the plugin to be installed. Here is an example of the input for the plugin ecommerce "tutor-ecommerce".|
